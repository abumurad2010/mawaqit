...
const { normalizeSearchText } = require('../lib/city-search-normalize');
...
async function main() {
  ...
  const cities = srcCities.map((c, i) => {
    ...
    // Search tokens = every UNIQUE NORMALIZED name we have for this city. Must use
    // the exact same normalizer LocationModal.tsx applies to the query so Arabic /
    // Persian yeh, alef variants, harakat, ta marbuta, etc. all fold identically.
    const s = new Set();
    for (const lang of LOCALES) {
      const name = i18n[lang];
      if (!name) continue;
