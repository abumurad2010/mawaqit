...
function hizbLabel(rubElHizb: number, isAr: boolean): string {
  ...
  if (isAr) {
    const arFrac = ['', '¼ ', '½ ', '¾ '];
    return `${arFrac[q]}حزب ${toArabicIndic(hizb)}`;
  }
  return `${fractions[q]}Hizb ${hizb}`;
}
      ...
      <View style={[banner.outer, { borderColor: color }]}>
        <View style={[banner.inner, { borderColor: color }]}>
          ...
          <View style={banner.center}>
            <Text style={[banner.surahName, { color: textColor, fontFamily: 'Amiri_700Bold', fontSize: Math.min(20, height * 0.55) }]}>
              سُورَةُ {meta.arabic}
            </Text>
            <Text style={[banner.meta, { color: mutedColor, fontFamily: 'Amiri_400Regular', fontSize: Math.min(11, height * 0.28) }]}>
...
const banner = StyleSheet.create({
  ...
  meta: { marginTop: 1 },
  sideOrn: { alignItems: 'center', justifyContent: 'center', gap: 2, width: 16 },
  orn: { width: 4, height: 4, borderRadius: 2 },
  ornLine: { width: 1, height: 14 },
});

/* ── One Mushaf page, fits the viewport exactly ── */
...
function MushafLineView({
  ...
  // verse line: render each segment with optional end marker and tap handler
  return (
    <View style={{ height, justifyContent: 'center' }}>
      <Text
        style={{
      ...
      >
        {line.segments.map((seg, si) => {
          const highlighted = highlightTarget?.surah === seg.surahNum && highlightTarget?.ayah === seg.ayahNum;
          const bookmarked = isBookmarked(seg.surahNum, seg.ayahNum);
          const segText = seg.text + (seg.isAyahEnd ? '' : ' ');
          return (
            <Text
              key={si}
              suppressHighlighting
              onPress={() => onAyahTap({ surah: seg.surahNum, ayah: seg.ayahNum })}
              style={{
                color: highlighted ? tintColor : textColor,
                backgroundColor: highlighted ? tintColor + '33' : undefined,
              }}
            >
              {segText}
              {seg.isAyahEnd && (
                <Text style={{ color: bookmarked ? '#C8860A' : tintColor, fontSize: fontSize * 0.85 }}>
...
/* ── Tap-on-ayah modal: translation + transliteration + bookmark ── */
function AyahDetailModal({
  target, visible, onClose, lang, translitLang, colors, isDark,
...
export default function QuranReaderScreen() {
  ...
  // Search-result navigation: jump to the page containing the ayah and highlight it
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const page = getMushafPage(pageNum);
    if (page) {
      // Pick a representative surah from the page (first verse line)
  ...
  // Compute viewport dimensions for each page
  ...
  // Pick a comfortable base font; AmiriQuran needs decent vertical room for diacritics.
  const baseFontSize = Math.min(28, Math.max(14, lineHeight * 0.55));

  const currentPage = getMushafPage(pageNum);
  const juzNum = currentPage?.juz ?? 1;
  const rubElHizb = currentPage?.rubElHizb ?? 1;

  // Display surah: the first surah whose verse appears on the page (preferred), else header surah
  ...
  const renderItem = useCallback(({ item }: { item: number }) => (
    ...
    />
  ), [pageWidth, pageHeight, lineHeight, baseFontSize, C, isDark, highlightTarget, isBookmarked]);
  ...
  return (
    <View style={[styles.root, { backgroundColor: bgColor }]}>
      ...
      {/* Pages — horizontal FlatList, paging enabled */}
      <FlatList
        ref={listRef}
        data={data}
      ...
      {/* Footer: page number with hizb mark */}
