...
/**
 * Shared label component. The help ⓘ sits IMMEDIATELY next to its label —
 * never beside a control, never on a value line. Fixed 6-px gap between label
 * and icon, same size (18), same vertical alignment.
 *
 * Defined at module scope (not inside SettingsScreen) so it is a stable
 * component identity across renders — React would otherwise treat every render
 * of the enclosing screen as a NEW component type and unmount+remount all
 * consumers on every keystroke. `isRtl`, `textColor` and `textMutedColor` are
 * passed as props rather than closed over so the component doesn't depend on
 * the screen's render scope.
 *
 * RTL mirroring: this component uses its own `flexDirection: row-reverse` when
 * `isRtl` is true, so the icon stays visually "after" the label in reading
 * order in both directions.
 */
const LabelWithHelp = React.memo(function LabelWithHelp({
  label, onHelp, style, numberOfLines, isRtl, textColor, textMutedColor,
}: {
  label: string;
  onHelp?: () => void;
  style?: any;
  ...
  return (
    <View style={{ flexDirection: isRtl ? 'row-reverse' : 'row', alignItems: 'center', gap: 6, flexShrink: 1 }}>
      <Text
        style={[labelHelpStyles.label, { color: textColor, fontFamily: isRtl ? 'Amiri_400Regular' : SANS, textAlign: isRtl ? 'right' : 'left' }, style]}
      ...
      {onHelp && (
        <Pressable onPress={onHelp} hitSlop={8}>
          <MaterialCommunityIcons name="help-circle-outline" size={18} color={textMutedColor} />
        </Pressable>
      )}
    </View>
  );
});
// Local style dict — LabelWithHelp is module-scope, so it can't reach the
// screen's `styles.settingLabel`. This mirrors that entry (fontSize 15, fontWeight 400).
const labelHelpStyles = StyleSheet.create({
...
export default function SettingsScreen() {
  ...
  const HELP: Record<string, Record<HelpKey, string>> = {
    ...
    bn: {
      ...
      adjustAllPrayerTimes: 'বন্ধ থাকলে শুধু মাগরিব সমন্বয় প্রযোজ্য হয়। চালু করলে পাঁচ ওয়াক্ত নামাজের প্রতিটির জন্য একটি প্লাস-মাইনাস নিয়ন্ত্রণ দেখা যায়, প্রতিটি ০ থেকে শুরু।',
  ...
  const Row = ({
    ...
    <View style={[
      ...
      <LabelWithHelp label={label} onHelp={onHelp} isRtl={isRtl} textColor={C.text} textMutedColor={C.textMuted} />
      ...
      >
        ...
        <View style={[styles.card, { backgroundColor: C.backgroundCard, borderColor: C.separator }]}>
          ...
          >
            <View style={{ flexDirection: isRtl ? 'row-reverse' : 'row', alignItems: 'center', gap: 10, flex: 1 }}>
              ...
              <LabelWithHelp
                label={tr.default_tab_label}
                onHelp={() => { Haptics.selectionAsync(); Alert.alert(tr.default_tab_help_title, tr.default_tab_help_body); }}
               isRtl={isRtl} textColor={C.text} textMutedColor={C.textMuted}
              />
            </View>
            ...
            <View style={[styles.settingRow, { borderBottomWidth: 0, flexDirection: 'column', alignItems: 'stretch', gap: 8 }]}>
              <View style={{ flexDirection: isRtl ? 'row-reverse' : 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                <View style={{ flex: 1 }}>
                  <LabelWithHelp
                    label={tr.dstSetting}
                    onHelp={() => showHelp(tr.dstSetting, HELP[lang]?.dst ?? HELP['en'].dst)}
                   isRtl={isRtl} textColor={C.text} textMutedColor={C.textMuted}
              />
                </View>
        ...
        <View style={[styles.card, { backgroundColor: C.backgroundCard, borderColor: C.separator }]}>
          ...
          >
            <View style={{ flex: 1, gap: 2 }}>
              <LabelWithHelp
                ...
                onHelp={() => showHelp(tr.calculationMethod, HELP[lang]?.calcMethod ?? HELP['en'].calcMethod)}
               isRtl={isRtl} textColor={C.text} textMutedColor={C.textMuted}
              />
          ...
          <View style={[styles.settingRow, { borderBottomWidth: 0, flexDirection: 'column', alignItems: 'stretch', gap: 6 }]}>
            <View style={{ flexDirection: isRtl ? 'row-reverse' : 'row', alignItems: 'center', justifyContent: 'space-between' }}>
              <LabelWithHelp
                label={tr.maghribSafetyMargin}
                onHelp={() => showHelp(tr.maghribSafetyMargin, HELP[lang]?.maghrib ?? HELP['en'].maghrib)}
               isRtl={isRtl} textColor={C.text} textMutedColor={C.textMuted}
              />
            </View>
          ...
          <View style={[styles.compactRow, { borderBottomWidth: draftPerPrayerEnabled ? StyleSheet.hairlineWidth : 0, borderBottomColor: C.separator, flexDirection: isRtl ? 'row-reverse' : 'row' }]}>
            <View style={{ flex: 1 }}>
              <LabelWithHelp
                label={tr.adjustAllPrayerTimes as string}
                onHelp={() => showHelp(tr.adjustAllPrayerTimes as string, HELP[lang]?.adjustAllPrayerTimes ?? HELP['en'].adjustAllPrayerTimes)}
                numberOfLines={2}
               isRtl={isRtl} textColor={C.text} textMutedColor={C.textMuted}
              />
            </View>
          ...
          <View style={[styles.compactRow, { borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: C.separator, flexDirection: isRtl ? 'row-reverse' : 'row' }]}>
            <View style={{ flex: 1 }}>
              <LabelWithHelp
                label={tr.firstAdhanSetting}
                onHelp={() => showHelp(tr.firstAdhan, HELP[lang]?.firstAdhan ?? HELP['en'].firstAdhan)}
               isRtl={isRtl} textColor={C.text} textMutedColor={C.textMuted}
              />
            </View>
            <View style={{ flexDirection: isRtl ? 'row-reverse' : 'row', alignItems: 'center', gap: 6 }}>
          ...
          <View style={[styles.compactRow, { borderBottomWidth: 0, flexDirection: isRtl ? 'row-reverse' : 'row' }]}>
            <View style={{ flex: 1 }}>
              <LabelWithHelp
                label={tr.prePrayerReminder as string}
                onHelp={() => showHelp(tr.prePrayerReminder as string, HELP[lang]?.prePrayer ?? HELP['en'].prePrayer)}
               isRtl={isRtl} textColor={C.text} textMutedColor={C.textMuted}
              />
            </View>
            <Pressable
        ...
        <View style={[styles.card, { backgroundColor: C.backgroundCard, borderColor: C.separator }]}>
          ...
          <View style={[styles.compactRow, { borderBottomWidth: 1, borderBottomColor: C.separator, flexDirection: isRtl ? 'row-reverse' : 'row' }]}>
            <View style={{ flex: 1 }}>
              <LabelWithHelp
                label={tr.dhuha}
                onHelp={() => showHelp(tr.dhuha, HELP[lang]?.dhuha ?? HELP['en'].dhuha)}
               isRtl={isRtl} textColor={C.text} textMutedColor={C.textMuted}
              />
            </View>
          ...
          <View style={[styles.compactRow, { borderBottomWidth: 1, borderBottomColor: C.separator, flexDirection: isRtl ? 'row-reverse' : 'row' }]}>
            <View style={{ flex: 1 }}>
              <LabelWithHelp
                label={tr.qiyam}
                onHelp={() => showHelp(tr.qiyam, HELP_QIYAM[lang] ?? HELP_QIYAM['en'])}
               isRtl={isRtl} textColor={C.text} textMutedColor={C.textMuted}
              />
            </View>
          ...
          <View style={[styles.compactRow, { borderBottomWidth: isNearEid ? 1 : 0, borderBottomColor: C.separator, flexDirection: isRtl ? 'row-reverse' : 'row' }]}>
            <View style={{ flex: 1 }}>
              <LabelWithHelp
                label={tr.jumuahTimeSetting as string}
                onHelp={() => showHelp(tr.jumuahTimeSetting as string, HELP[lang]?.jumuahTime ?? HELP['en'].jumuahTime)}
               isRtl={isRtl} textColor={C.text} textMutedColor={C.textMuted}
              />
            </View>
        ...
        <View style={[styles.card, { backgroundColor: C.backgroundCard, borderColor: C.separator }]}>
          <View style={[styles.settingRow, { borderBottomWidth: 0, flexDirection: isRtl ? 'row-reverse' : 'row' }]}>
            <View style={{ flexDirection: isRtl ? 'row-reverse' : 'row', alignItems: 'center', gap: 10, flex: 1 }}>
              ...
              <View style={{ flex: 1 }}>
                <LabelWithHelp
                  label={tr.thikr_reminders_label}
                  onHelp={() => { Haptics.selectionAsync(); Alert.alert(tr.thikr_reminders_help_title, tr.thikr_reminders_help_body); }}
                 isRtl={isRtl} textColor={C.text} textMutedColor={C.textMuted}
              />
