...
interface AppSettings {
  ...
  /** True once the round-5 override cleanup has run (dropped every
   *  calcMethodByCountry entry and every SavedCity.calcMethod that was
   *  written by the stale-draft bug in rounds 2–3). Set once, never reset. */
  calcMethodMigrationV2?: boolean;
  asrMethod: AsrMethod;
...
interface AppContextValue extends AppSettings {
  ...
  /** Remove any per-city or per-country override for the current location so
   *  the resolver falls back to getRecommendedMethod on the next render. Used by
   *  the "Use recommended" affordance in Settings. */
  clearEffectiveCalcMethodOverride: () => Promise<void>;
  /** True iff an explicit override exists for the current location — either
   *  currentSavedCity.calcMethod or calcMethodByCountry[effectiveCountryCode].
   *  An override equal to the recommended method still counts as an override:
   *  the user's INTENT to pin was recorded. Used by the Settings source-hint. */
  hasCalcMethodOverride: boolean;
  /** The location actually used by the resolver: manualLocation when
   *  locationMode === 'manual', else the raw GPS `location`. Exposed so
   *  consumers like the Settings recommended-method label can pass the same
   *  latitude the resolver used. */
...
export function AppProvider({ children }: { children: ReactNode }) {
    ...
    (async () => {
      try {
        ...
        if (s) {
          ...
          // 1.3.8 round-5 migration: nuke every calc-method override.
          //
          // Rounds 2 and 3 shipped a stale-draft bug (Settings save called
          // setEffectiveCalcMethod based on a frozen useState from mount, so
          // whenever the location shifted while the screen was mounted the
          // save wrote an unwanted per-country / per-city override under the
          // NEW country's key). Any override currently on disk is more likely
          // a residue of that bug than a deliberate user choice. Clearing them
          // lets the resolver fall back to getRecommendedMethod, which is
          // correct for nearly every user. Version-flagged so it runs once.
          const runV2Migration = !parsed.calcMethodMigrationV2;
          if (runV2Migration) {
            const cleared = {
              countries: Object.keys(parsed.calcMethodByCountry ?? {}),
              cities: (Array.isArray(parsed.savedCities) ? parsed.savedCities : [])
                .filter((c: SavedCity) => c.calcMethod)
                .map((c: SavedCity) => ({ id: c.id, city: c.city, calcMethod: c.calcMethod })),
            };
            if (cleared.countries.length || cleared.cities.length) {
              console.log('[migration v2] clearing calc-method overrides:', JSON.stringify(cleared));
            }
            parsed.calcMethodByCountry = {};
            if (Array.isArray(parsed.savedCities)) {
              parsed.savedCities = parsed.savedCities.map((c: SavedCity) => {
                if (!c.calcMethod) return c;
                const { calcMethod: _drop, ...rest } = c;
                return rest;
              });
            }
            parsed.calcMethodMigrationV2 = true;
          }
          ...
          setSettings(merged);
          // Persist the migration flag immediately so a launch where the user
          // never triggers updateSettings still records that the v2 cleanup
          // ran. Without this, the block above would re-run on every launch
          // against whatever AsyncStorage still holds — permanently wiping any
          // override the user deliberately sets after the migration ran.
          if (runV2Migration) {
  ...
  const hasCalcMethodOverride: boolean = Boolean(currentSavedCity?.calcMethod || countryOverride);
  ...
  const clearEffectiveCalcMethodOverride: AppContextValue['clearEffectiveCalcMethodOverride'] = async () => {
    const patch: Partial<AppSettings> = {};
    if (currentSavedCity?.calcMethod) {
      patch.savedCities = settings.savedCities.map(x => {
        if (x.id !== currentSavedCity.id) return x;
        const { calcMethod: _drop, ...rest } = x;
        return rest;
      });
    }
    if (effectiveCountryCode && settings.calcMethodByCountry?.[effectiveCountryCode]) {
      const { [effectiveCountryCode]: _drop, ...rest } = settings.calcMethodByCountry;
      patch.calcMethodByCountry = rest;
    }
  ...
  const value = useMemo<AppContextValue>(
    () => ({
      ...
      clearEffectiveCalcMethodOverride,
      hasCalcMethodOverride,
      effectiveLocation,
    }),
    [settings, isDark, isRtl, colors, resolvedSecondLang, location, maghribBase, maghribOffset, maghribEffective, maghribUserAdj, effectiveCountryCode, effectiveCalcMethod, hasCalcMethodOverride, currentSavedCity, locationUtcOffset, locationTimezone, dstOverrideOffset, dstShiftMs, dstResolved, bookmarks, lastReadPage, lastReadSurah, translitLastSurah, translitLastPage, quranNavTarget],
  );
