...
/* ──────────────────────────────────────────────────────────────────────
 * Madani Mushaf 15-line page layout — sourced from Quran.com word-by-word
 * data (line_number per word). Bundled in assets/quran-pages.json so the
 * reader can render each page exactly as the printed Madani Mushaf does.
 * ───────────────────────────────────────────────────────────────────── */

interface RawLineSegment { s: number; a: number; t: string; e?: number }
interface RawPageLine {
  type: 'verse' | 'header' | 'bismillah' | 'blank';
  segments?: RawLineSegment[];
  surah?: number;
}
interface RawMushafPage {
  juz: number;
  rubElHizb: number;
  lines: RawPageLine[];
}

const MUSHAF_PAGES_RAW = require('../assets/quran-pages.json') as Record<string, RawMushafPage | null>;
...
export type MushafLine =
  | { type: 'verse'; segments: MushafLineSegment[] }
  | { type: 'header'; surah: number }
  | { type: 'bismillah'; surah: number }
  | { type: 'blank' };
...
const _mushafCache: Record<number, MushafPage | null> = {};
function buildMushafPage(p: number): MushafPage | null {
  if (p < 1 || p > 604) return null;
  if (_mushafCache[p] !== undefined) return _mushafCache[p];
  const raw = MUSHAF_PAGES_RAW[String(p)];
  if (!raw) { _mushafCache[p] = null; return null; }
  const lines: MushafLine[] = raw.lines.map(l => {
    if (l.type === 'verse' && l.segments) {
      return {
        type: 'verse',
        segments: l.segments.map(s => ({
          surahNum: s.s, ayahNum: s.a, text: s.t, isAyahEnd: !!s.e,
        })),
      };
    }
    if (l.type === 'header') return { type: 'header', surah: l.surah ?? 0 };
    if (l.type === 'bismillah') return { type: 'bismillah', surah: l.surah ?? 0 };
    return { type: 'blank' };
  });
  const page: MushafPage = { juz: raw.juz, rubElHizb: raw.rubElHizb, lines };
  _mushafCache[p] = page;
  return page;
}

export function getMushafPage(pageNum: number): MushafPage | null {
  return buildMushafPage(pageNum);
}

/** Hizb number (1-60) from rub-el-hizb (1-240). */
export function getHizbNumber(rubElHizb: number): number {
  return Math.floor((rubElHizb - 1) / 4) + 1;
}

/** Which quarter (0=full hizb start, 1=¼, 2=½, 3=¾) of the current hizb this rub-el-hizb marks. */
export function getHizbQuarter(rubElHizb: number): number {
  return (rubElHizb - 1) % 4;
}
