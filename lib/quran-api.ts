 ...
 * reader can render each page in the Madani Mushaf form.
 * ───────────────────────────────────────────────────────────────────── */
...
export function getMushafPage(pageNum: number): MushafPage | null {
  ...
  if (_mushafCache[pageNum] !== undefined) return _mushafCache[pageNum];
  const raw = MUSHAF_PAGES_RAW[String(pageNum)];
  if (!raw) { _mushafCache[pageNum] = null; return null; }
